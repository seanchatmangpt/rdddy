...
def create_app(index_root: Path, api_key: str):
    ...
    _app.add_middleware(
        middleware_class=BaseHTTPMiddleware, dispatch=check_basic_api_key_auth_header
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--index-root", required=True, help="Root directory that should be indexed"
    )
    parser.add_argument(
        "--api-key", required=False, help="API key for the Smolex Server"
    )
    ...
    if args.api_key is None:
        ...
        print(
            f"Random API key generated. Use --api-key to set a custom API key: {args.api_key}"
        )
