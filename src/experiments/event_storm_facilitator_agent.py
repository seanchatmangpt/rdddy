...
class EventStormFacilitatorAgent(AbstractActor):
    ...
    async def handle_start_workshop(self, message: AbstractCommand):
        ...
        await self.publish(
            AbstractEvent(actor_id=self.actor_id, content="Workshop started")
        )
    ...
    async def handle_query_workshop(self, message: AbstractQuery):
        ...
        await self.publish(
            AbstractEvent(actor_id=self.actor_id, content=response_content)
        )
    ...
    async def handle_exit_workshop(self, message: AbstractCommand):
        ...
        await self.send(
            self, AbstractEvent(actor_id=self.actor_id, content="Workshop exited")
        )
...
class ParticipantAgent(AbstractActor):
    ...
    async def handle_join_workshop(self, message: AbstractCommand):
        ...
        await self.publish(
            AbstractEvent(actor_id=self.actor_id, content="Joined the workshop")
        )
