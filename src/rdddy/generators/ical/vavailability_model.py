...
class VAVAILABILITYModel(BaseModel):
    ...
    exdate: datetime = Field(default=None, title="", description="A list of dates to exclude from the recurrence rule.")
    uid: str = Field(default=None, title="", description="A unique identifier for the event.", required)
    ...
    sequence: int = Field(default=0, title="", description="The sequence number of the event.", ge=0)
    status: str = Field(default=CONFIRMED, title="", description="The status of the event.", allowed_values=['CONFIRMED', 'TENTATIVE', 'CANCELLED'])
    ...
    categories: List[str] = Field(default=None, title="", description="A list of categories for the event.")
