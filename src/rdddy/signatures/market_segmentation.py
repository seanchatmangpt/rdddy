...
class MarketSegmentation(Signature):
    ...
    customer_demographics = InputField(
        desc="Information about the characteristics of customers."
    )
    purchase_history = InputField(desc="Records of past purchases made by customers.")

    segment_labels = OutputField(
        desc="Labels indicating which segment each customer belongs to."
    )
